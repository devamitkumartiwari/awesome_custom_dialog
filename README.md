...
- **Staggered lists & grids**: `ACDStaggeredList`/`ACDStaggeredGrid`/`ACDStaggeredSliverList`/`ACDStaggeredSliverGrid` accept one shared `ACDStaggerOptions` (`interval`, `itemDuration`, `curve`, `effect`, `exitEffect`, `reverseOnExit`) for full control over the cascade, plus `reverse`/`scrollDirection` and RTL-aware effects via `ACDMotionEffect.slideInFromStart`/`slideInFromEnd`. `listOfACDListTile()`/`listOfACDRadioButton()`/`listOfACDCheckbox()` and the base `ACDDialog` itself (`contentStagger`) accept the same options.
- **Dialog shape & elevation**: the base `ACDDialog` accepts a full `shape` (`ShapeBorder`) escape hatch, `elevation`/`shadowColor`/`boxShadow`, and `backgroundGradient`/`backgroundImage` — the same customization surface its toast/snackbar/switch/slide-action siblings already had.
- **Gradients & elevation, consistently**: as a rule across the whole package, any widget with a background color also accepts a matching `Gradient?` override, and every widget — including the dialog core — now exposes an `elevation`/`boxShadow` pair.
- **Corners**: `borderRadius` is available on dialogs, images, text fields, toasts, snackbars, lists, buttons, and every new widget above. For a dialog that only wants *some* corners rounded (like a side panel sitting flush against an edge), pass a full `cornerRadius` instead — or a full `shape` (`ShapeBorder`) for anything beyond rounded rectangles (pill, notched, superellipse...):
  ```dart
  ACDDialog().build(context)
...
- **Staggered lists & grids**: `ACDStaggeredList`/`ACDStaggeredGrid` — and their `ACDStaggeredSliverList`/`ACDStaggeredSliverGrid` counterparts for composing inside a `CustomScrollView` — cascade each row/cell in by index, with a staggered exit via `visible`, all driven by one `ACDStaggerOptions`.
...
| `ACDStaggeredList` / `ACDStaggeredGrid` | Staggered-entrance/exit `ListView`/`GridView`, driven by one shared `ACDStaggerOptions`. |
| `ACDStaggeredSliverList` / `ACDStaggeredSliverGrid` | `Sliver` counterparts for composing inside a `CustomScrollView`. |
| `ACDStaggerOptions` | Shared timing/effect config (`interval`, `itemDuration`, `curve`, `effect`, `exitEffect`, `reverseOnExit`) for the staggered widgets above and `.contentStagger`/`stagger` below. |
...
| `.shape` / `.elevation` / `.boxShadow` | Full custom `ShapeBorder`, drop-shadow elevation, and an explicit shadow override for the dialog card. |
| `.backgroundGradient` / `.backgroundImage` | Gradient or image/texture fill for the dialog card, on top of `.backgroundColor`. |
| `.contentStagger` | Staggers every top-level widget added to the dialog in one after another, using `ACDStaggerOptions`. |
